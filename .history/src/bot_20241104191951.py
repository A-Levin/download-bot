    ...
    try:
        ...
        try:
            async def upload_progress(current, total):
                ...
                logger.info(f"Upload progress: {progress:.1f}% ({current/(1024*1024):.1f}MB/{total/(1024*1024):.1f}MB)")
                if progress % 10 < 1:  # Обновляем каждые 10%
                    await status_message.edit_text(f"Отправка: {progress:.1f}%")

            with open(filename, 'rb') as video_file:
                logger.info("File opened for upload")
